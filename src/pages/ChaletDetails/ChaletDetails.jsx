    ...
    return (
        ...
        <div className="bg-blue-50 my-10 mx-4 sm:mx-8">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="w-full md:w-[40%] px-2 sm:px-4">
              <h1 className="text-2xl sm:text-3xl font-bold my-4 sm:my-6">{chalet.title}</h1>
              <p className="text-sm sm:text-base">{chalet.description}</p>
              <h1 className="text-2xl sm:text-3xl font-bold text-[#0061E0] my-6 sm:my-8">
                {chalet.price} / ليله
              </h1>
            </div>
      
            <div className="w-full md:w-[40%] my-6 sm:my-10 px-2 sm:px-4">
              <img
                src={chalet.img}
                alt={chalet.title}
                className="w-full h-[200px] sm:h-full object-cover rounded-lg"
              />
              <div className="flex flex-wrap justify-between my-4 sm:my-6">
                {chalet.gallery.map((img, index) => (
                  <img
                    key={index}
                    src={img}
                    alt={`Gallery ${index + 1}`}
                    className="w-[48%] sm:w-[32%] rounded-xl mb-4"
                  />
                ))}
              </div>
            </div>
          ...
          <div className="w-full md:w-1/2 space-y-4 sm:space-y-6 px-4 pb-8">
            {/* المرافق */}
            ...
            {openSection === "المرافق" && (
              <div className="mt-2 sm:mt-4 text-[#101828] bg-white p-4 sm:p-6">
            ...
            {openSection === "شروط الحجز" && (
              <div className="text-[#101828] bg-white p-4 sm:p-6">
                <div className="flex items-center mb-4">
        ...
        <div className="my-6 sm:my-8 px-4 sm:px-8">
          <button className="bg-gradient-to-l from-[#48BB78] to-[#1A71FF] text-white py-2 sm:py-3 px-6 sm:px-8 rounded-lg text-sm sm:text-lg hover:bg-green-600">
